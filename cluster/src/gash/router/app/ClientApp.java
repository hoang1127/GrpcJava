...
public class ClientApp implements CommListener {
	...
	private void menu(String cmd, String filename) {

			if (cmd.equals("write")) {
				  mc.chunkAndSend(filename);
	...
	public static void main(String[] args) {
		...
		if (args.length > 0) {
			...
			String[] argsInstructions = args[0].split("-");
			connectedClusterId = argsInstructions[0];
		
			
			if (argsInstructions.length > 1) {
				command = argsInstructions[1];
			}
			if (argsInstructions.length > 2) {
				filename = argsInstructions[2];
			}
